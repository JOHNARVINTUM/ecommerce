...
@section('content')
    <div class="min-h-screen w-full overflow-x-hidden bg-[#f2f1eb] text-[#111]">
        ...
        <section class="relative overflow-hidden bg-black">
            <img src="{{ $heroImage }}" alt="{{ $category->name }}" class="h-[180px] w-full object-cover sm:h-[220px]">
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center px-6 text-center text-white">
                ...
                <p class="mt-3 text-sm sm:text-lg">{{ $category->headline ?: ($category->description ?: 'Browse available services in this category.') }}</p>
            </div>
        </section>

        <section class="px-5 py-10 sm:px-8 lg:px-10">
            <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-2xl font-bold sm:text-3xl">{{ $category->name }} Services</h2>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                @forelse($services as $service)
                    <article class="overflow-hidden rounded-xl border border-[#d8d7d0] bg-white shadow-[0_8px_18px_rgba(0,0,0,0.06)]">
                        <img src="{{ $service->thumbnail_url }}" alt="{{ $service->title }}" class="h-44 w-full object-cover">
                        <div class="p-4">
                ...
                @empty
                    <p class="text-base text-[#5a5a5a]">No services found in this category.</p>
                @endforelse
            </div>
