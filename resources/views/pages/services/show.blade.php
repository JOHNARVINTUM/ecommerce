    ...
    <div class="flex min-h-screen w-full flex-col overflow-x-hidden bg-[#f2f1eb] text-[#111]">
        ...
        <section class="flex-1 px-5 py-8 sm:px-8 lg:px-10">
            ...
            <div class="mb-7 text-xs text-[#8a8a8a] sm:text-sm">
                ...
                <span class="mx-2">></span>
                <a href="{{ route('services.category', $service->category) }}" class="hover:text-black">{{ $service->category->name ?? 'Category' }}</a>
                <span class="mx-2">></span>
                <span class="font-semibold text-[#3a3a3a]">{{ $service->title }}</span>
            </div>
            ...
            <div class="grid gap-8 xl:grid-cols-[1.05fr_0.95fr]" id="service-detail-layout">
                <div>
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-[#7b7b7b]">Expert in {{ $service->category->name ?? 'General Services' }}</p>
                    ...
                    <div class="mt-3 grid grid-cols-5 gap-2" id="gallery-thumbs">
                        @foreach ($gallery as $index => $image)
                            <button type="button" data-index="{{ $index }}" class="gallery-thumb overflow-hidden rounded-sm border border-[#d2d2cd] {{ $index === 0 ? 'ring-2 ring-[#111] is-active' : '' }}" aria-label="Show image {{ $index + 1 }}">
                    ...
                    <div class="mt-8 rounded-sm border border-[#9f9f9a] bg-[#f4f4ef] p-5">
                        <h2 class="text-sm font-semibold uppercase tracking-wide text-[#6f6f6f]">Get to know {{ $providerName }}</h2>
                ...
                <aside>
                    <p class="text-xs text-[#8a8a8a]">{{ $providerName }}</p>
                    ...
                    <div class="mt-4 flex items-end justify-between border-b border-[#d0d0cb] pb-4">
                        <p class="text-4xl font-semibold text-[#1f1f1f]">{{ $service->currency ?: 'PHP' }} {{ number_format($service->price, 2) }}</p>
                        <p class="text-sm text-[#707070]">{{ (int) $service->sold_count }} Sold</p>
                    </div>

                    <div class="mt-6">
                        <h2 class="text-lg font-semibold">Description:</h2>
                        <p class="mt-2 text-sm leading-7 text-[#666]">{{ $service->short_description ?: $service->description }}</p>
                    </div>
                        ...
                        @auth
                            @if (auth()->user()->role === 'customer')
                                <form action="{{ route('cart.store', $service->slug) }}" method="POST" class="w-full sm:w-auto">
                                    @csrf
                                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-sm bg-[#111] px-8 py-3 text-sm font-semibold text-white hover:bg-black">
                                        Add To Cart
                                    </button>
                                </form>
                            @else
                            ...
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="inline-flex items-center justify-center rounded-sm bg-[#111] px-8 py-3 text-sm font-semibold text-white hover:bg-black">
                                Add To Cart
                            </a>
                        @endauth
                    </div>
        ...
        (() => {
            ...
            const setActive = (index) => {
                ...
                thumbs.forEach((thumb, thumbIndex) => {
                    ...
                    thumb.classList.toggle('ring-[#111]', active);
                });
            };
