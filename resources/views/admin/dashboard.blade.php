...
@section('content')
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-3">
        <div
            class="rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.25)] lg:col-span-2"
        ...
        >
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="inline-flex overflow-hidden rounded-lg border border-white/10 bg-white/[0.03]">
                    <button type="button" class="px-3 py-2 text-xs font-semibold"
                        :class="selectedRange === 'all' ? 'bg-white text-[#111]' : 'bg-transparent text-white/75'"
                        @click="selectedRange = 'all'">
                        All Time
                    </button>
                    <button type="button" class="border-l border-white/10 px-3 py-2 text-xs font-semibold"
                        :class="selectedRange === 'month' ? 'bg-white text-[#111]' : 'bg-transparent text-white/75'"
                        @click="selectedRange = 'month'">
                        This Month
                    </button>
                    <button type="button" class="border-l border-white/10 px-3 py-2 text-xs font-semibold"
                        :class="selectedRange === '7d' ? 'bg-white text-[#111]' : 'bg-transparent text-white/75'"
            ...
            <div class="relative mt-6 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <svg viewBox="0 0 640 280" class="h-64 w-full" @mouseleave="clearHover()">
                    <text x="22" y="18" class="fill-slate-300 text-[10px] font-semibold">Y: Orders</text>
                    <line x1="50" y1="20" x2="50" y2="240" stroke="#6b7280" stroke-width="1" />
                    <line x1="50" y1="240" x2="610" y2="240" stroke="#6b7280" stroke-width="1" />

                    <template x-for="tick in [0, 0.25, 0.5, 0.75, 1]" :key="tick">
                        <g>
                            <line :x1="50" :x2="610" :y1="240 - (220 * tick)" :y2="240 - (220 * tick)" stroke="#374151" stroke-width="1" />
                            <text x="8" :y="244 - (220 * tick)" class="fill-slate-300 text-[10px]" x-text="Math.round((current.max_count ?? 1) * tick)"></text>
                        </g>
                    </template>
                    ...
                    <template x-for="(point, index) in current.values" :key="'dot-' + index">
                        <g>
                            ...
                            <circle
                                ...
                                @mouseenter="setHover(index)"
                            ></circle>
                            <text :x="xPos(index)" :y="yPos(point) - 10" text-anchor="middle" class="fill-white text-[10px] font-semibold" x-text="point"></text>
                        </g>
                    </template>
                </svg>
        ...
        <div class="rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.25)]">
            ...
            <div class="mt-4 space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/65">Pending Orders</span>
                    <x-status-badge status="pending" />
                    <span class="text-sm font-semibold text-white">{{ $orderChartData['all']['status_counts']['pending'] ?? 0 }}</span>
                </div>

                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/65">Completed Orders</span>
                ...
                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/65">Cancelled Orders</span>
    ...
    <div class="mt-8 rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.25)]">
        <div class="flex items-center justify-between">
