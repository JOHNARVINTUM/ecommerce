    ...
    <div class="mx-auto max-w-3xl rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.25)]">
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-white/65">Update the account details, role, and profile basics.</p>
            </div>
            <a href="{{ route('admin.users.show', $user) }}" class="rounded-lg border border-white/20 bg-white/5 px-4 py-2 text-sm font-semibold text-white/90 transition hover:border-white/35 hover:bg-white/10">
                Back
            </a>
        </div>
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    <label for="password" class="mb-2 block text-sm font-medium text-white/85">New Password</label>
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="4" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">{{ old('bio', $user->profile->bio ?? '') }}</textarea>
