    ...
    <div class="mx-auto max-w-4xl rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.25)]">
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-white/65">Update service details, provider assignment, and media.</p>
            </div>
        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="provider_user_id" id="provider_user_id" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
                        @foreach ($providers as $provider)
                            <option value="{{ $provider->id }}" @selected(old('provider_user_id', $service->provider_user_id) == $provider->id)>{{ $provider->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $service->title) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
                ...
                <div>
                    ...
                    <input type="text" name="short_description" id="short_description" value="{{ old('short_description', $service->short_description) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">{{ old('description', $service->description) }}</textarea>
            ...
            <div class="grid gap-6 md:grid-cols-4">
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="price" id="price" value="{{ old('price', $service->price) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
                ...
                <div>
                    ...
                    <input type="text" name="currency" id="currency" value="{{ old('currency', $service->currency) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
                ...
                <div>
                    ...
                    <input type="number" min="1" name="delivery_time_days" id="delivery_time_days" value="{{ old('delivery_time_days', $service->delivery_time_days) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="revisions" id="revisions" value="{{ old('revisions', $service->revisions) }}" class="w-full rounded-xl border border-white/20 bg-white/5 px-4 py-3 text-white">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    <label for="thumbnail" class="mb-2 block text-sm font-medium text-white/85">Replace Thumbnail</label>
                    ...
                    @if ($service->thumbnail_path)
                        <p class="mt-2 text-xs text-white/55">Current thumbnail stored: {{ $service->thumbnail_path }}</p>
                    @endif
                </div>
                <div>
                    <label for="gallery_images" class="mb-2 block text-sm font-medium text-white/85">Add Gallery Images</label>
            ...
            @if (!empty($service->gallery_images))
                <div>
                    <p class="mb-3 text-sm font-medium text-white/85">Keep Existing Gallery Images</p>
                    <div class="space-y-2">
                        @foreach ($service->gallery_images as $imagePath)
                            <label class="flex items-center gap-3 text-sm text-white/75">
                                <input type="checkbox" name="keep_existing_gallery[]" value="{{ $imagePath }}" @checked(collect(old('keep_existing_gallery', $service->gallery_images))->contains($imagePath)) class="rounded border-white/30 bg-transparent text-indigo-400 focus:ring-indigo-300/50">
                                <span>{{ $imagePath }}</span>
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" @checked(old('is_active', $service->is_active)) class="rounded border-white/30 bg-transparent text-indigo-400 focus:ring-indigo-300/50">
